...
> **상태**: 전체 12 Phase 완료 (2026-04-04)

### Phase 1: 기반 구축 ✅
1. Next.js 16 프로젝트 초기화 (app/ 디렉토리)
2. Tailwind 4 + shadcn/ui 설정
3. 레이아웃 (Sidebar + TopBar + StatusBar)
4. API 프록시 설정 (next.config.ts rewrites)
5. Zustand 스토어 (auth)
6. 인증 연동 (모의/실전 로그인)
7. Claude Code CLI spawn 방식 검증 완료 (SDK는 CLI 전용이라 프로그래밍 API 미제공)

### Phase 2: 대시보드 + 포트폴리오 ✅
8. 대시보드 페이지 (잔고, 보유종목, 코스피/코스닥 지수)
9. 포트폴리오 페이지 (요약 카드, 도넛 차트, 보유종목 상세)

### Phase 3: 전략 빌더 ✅ (병렬 실행)
10. 프리셋 전략 브라우저 (카테고리 그리드)
11. 커스텀 전략 빌더 (5단계 위저드 + YAML 프리뷰)
12. 프리셋 상세 (Sheet 패널)

### Phase 4: 백테스트 ✅ (병렬 실행)
13. 백테스트 실행 페이지 (전략 선택, 파라미터 슬라이더)
14. 수익 곡선 차트 (Recharts AreaChart + 벤치마크)
15. 성과 지표 그리드 (수익률, 샤프, MDD, 승률)
16. 거래 내역 테이블

### Phase 5: 트레이딩 ✅ (병렬 실행)
17. 3패널 레이아웃 (전략/신호/잔고)
18. 신호 생성 (BUY/SELL/HOLD + 강도 바)
19. 주문 확인 다이얼로그
20. 미체결 주문 관리

### Phase 6: AI 챗봇 ✅ (병렬 실행)
21. Claude Code CLI spawn 연동 (/api/chat)
22. 채팅 UI (마크다운 렌더링, react-markdown)
23. 빠른 명령 버튼 (잔고 확인, 전략 목록, 신호 확인, 백테스트)

### Phase 7: 부가 기능 ✅ (3개 병렬 실행)
24. 종목 탐색기 (검색, 현재가, 호가 테이블)
25. 설정 페이지 (인증, 테마, 서버 상태, 마스터 파일)
26. 테마 전환 (다크/라이트/시스템, localStorage 저장)
27. 반응형 모바일 대응 (그리드, 터치 타겟, 가로 스크롤)

### Phase 8: 워치리스트 + 차트 ✅ (병렬 실행)
28. 워치리스트 CRUD (JSON 파일 저장)
29. 종목별 실시간 가격 표시
30. 가격 차트 컴포넌트 (Recharts AreaChart)

### Phase 9: 매매 저널 + 리스크 ✅ (병렬 실행)
31. 매매 저널 CRUD (SQLite + Drizzle ORM)
32. 일일 요약, 태그 필터
33. 리스크 대시보드 (집중도 경고, 포트폴리오 분석)

### Phase 10-11: 성과 추적 + 도구 ✅ (병렬 실행)
34. 전략 성과 추적 (일간 수익률 차트, 실전 vs 백테스트 비교)
35. 포지션 사이징 계산기 (고정비율/리스크/켈리)

### Phase 12: 시장 캘린더 + 뉴스 ✅ (병렬 실행)
36. 시장 캘린더 (월간 그리드, 이벤트 CRUD)
37. 뉴스 메모 (JSON 저장, 감성 태그)
